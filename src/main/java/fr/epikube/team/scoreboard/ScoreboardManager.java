...
/**
 * @author SamaGames and modification by @IsiZ
 */
public class ScoreboardManager {
    ...
    public ScoreboardManager() {
        ...
        glowingTask = Epikube.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() -> {
            String ip = colorIpAt();
            for (PersonalScoreboard scoreboard : scoreboards.values())
                Epikube.getInstance().getExecutorMonoThread().execute(() -> scoreboard.setLines(ip));
        }, 80, 80, TimeUnit.MILLISECONDS);

        reloadingTask = Epikube.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() -> {
            for (PersonalScoreboard scoreboard : scoreboards.values())
                Epikube.getInstance().getExecutorMonoThread().execute(scoreboard::reloadData);
